...
1. 安装 `HermesAgent-0.1.1-fnOS-x86_64.fpk`。
2. 在 Agent 配置中选择一个 fnOS 授权目录，并配置模型/API 和消息平台。
3. 安装 `HermesStudio-0.1.1-fnOS-x86_64.fpk`。
...
构建流程还会对 FPK 外层包和内部 `app.tgz` 执行解包、gzip、tar 路径和内容校验；结果保存到 `dist/fpk-payload-check.log`，并随 Release 一起发布。

## 版本与 Release

- 包版本从 `0.1.0` 开始，当前日志修复版本为 `0.1.1`，由 `FPK_VERSION` 控制。
